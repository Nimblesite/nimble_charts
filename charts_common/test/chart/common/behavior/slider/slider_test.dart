...
class SliderMockChart extends MockChart<num> {
  GestureListener? lastGestureListener;
  LifecycleListener<num>? lastLifecycleListener;
  ...
  @override
  LifecycleListener<num> addLifecycleListener(
      LifecycleListener<num>? listener) {
...
class MockDomainAxis extends Mock implements NumericAxis {
  @override
  double getDomain(double location) => location / 20.0;

  @override
  double? getLocation(num? domain) => (domain ?? 0) * 20.0;
}

void main() {
  late SliderMockChart chart;
  late MockDomainAxis domainAxis;
  late MockDomainAxis measureAxis;
  late ImmutableSeries<num> series1;
  late DatumDetails<num> details1;
  late DatumDetails<num> details2;
  late DatumDetails<num> details3;

  late SliderTester<num> tester;

  Slider<num> makeBehavior(
    SelectionTrigger eventTrigger, {
    Point<double>? handleOffset,
    Rectangle<int>? handleSize,
    double? initialDomainValue,
    SliderListenerCallback<num>? onChangeCallback,
    bool snapToDatum = false,
    SliderHandlePosition handlePosition = SliderHandlePosition.middle,
  }) {
    // Set up mocks that attachTo needs before calling it.
    when(chart.domainAxis).thenReturn(domainAxis);
    when(chart.getMeasureAxis()).thenReturn(measureAxis);

    final behavior = Slider<num>(
      eventTrigger: eventTrigger,
      initialDomainValue: initialDomainValue,
      onChangeCallback: onChangeCallback,
      snapToDatum: snapToDatum,
      style: SliderStyle(
        handleOffset: handleOffset ?? const Point<double>(0, 0),
    ...
    tester = SliderTester<num>(behavior)
      ..layout(
        const Rectangle<int>(0, 0, 200, 200),
        const Rectangle<int>(0, 0, 200, 200),
      );
  ...
  setUp(() {
    chart = SliderMockChart();

    domainAxis = MockDomainAxis();

    measureAxis = MockDomainAxis();

    series1 = MutableSeries<num>(Series<dynamic, num>(
      id: 'mySeries1',
      data: <dynamic>[],
      domainFn: (_, __) => 0,
      measureFn: (_, __) => null,
  ...
  group('Slider trigger handling', () {
    test('can listen to tap and drag', () {
      ...
      expect(
          tester.handleBounds, equals(const Rectangle<int>(115, 90, 10, 20)));
    ...
    test('slider handle can render at top', () {
      ...
      const endPoint = Point<double>(120, 0);
      setupChart(
          forPoint: endPoint,
          isWithinRenderer: true,
          respondWithDetails: [details3]);
      ...
      expect(tester.domainValue, equals(5.0));
      expect(
          tester.handleBounds, equals(const Rectangle<int>(95, -10, 10, 20)));

      // Drag to first update point.
      chart.lastGestureListener!.onDragUpdate!(updatePoint1, 1);
      expect(tester.domainValue, equals(2.5));
      expect(
          tester.handleBounds, equals(const Rectangle<int>(45, -10, 10, 20)));
      ...
      expect(tester.domainValue, equals(6.0));
      expect(
          tester.handleBounds, equals(const Rectangle<int>(115, -10, 10, 20)));
  ...
  group('Slider manual control', () {
    ...
    test('can set domain and measure position when handle position is manual',
      ...
      expect(tester.domainValue, equals(1.0));
      expect(
          tester.handleBounds, equals(const Rectangle<int>(15, 190, 10, 20)));
      ...
      expect(tester.domainValue, equals(5.0));
      expect(
          tester.handleBounds, equals(const Rectangle<int>(95, 130, 10, 20)));
      ...
      expect(tester.domainValue, equals(7.5));
      expect(
          tester.handleBounds, equals(const Rectangle<int>(145, 50, 10, 20)));
    });
  });
